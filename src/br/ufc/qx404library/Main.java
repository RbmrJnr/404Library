...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        Book[] LivrosBiblioteca = {b1, b2, b3};

        System.out.println("System.Out:");
        BookOutputStream bosConsole = new BookOutputStream(System.out, LivrosBiblioteca, 3);
        bosConsole.enviarLivros();

        System.out.println("FileOutputStream:");
        FileOutputStream fos = new FileOutputStream("livros.txt");
        BookOutputStream bosArquivo = new BookOutputStream(fos, LivrosBiblioteca, 3);
        bosArquivo.enviarLivros();
        System.out.println("O Arquivo livros.txt foi gerado!");

        System.out.println("TCP:");
        Socket socket = new Socket("127.0.0.1", 8800);
        BookOutputStream bosTCP = new BookOutputStream(socket.getOutputStream(), LivrosBiblioteca, 3);
        bosTCP.enviarLivros();
        socket.close();
